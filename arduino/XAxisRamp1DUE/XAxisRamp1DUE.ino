...
// *** NEU / ÄNDERUNG: Timeout konfigurierbar: 0 = KEIN Timeout (unendlich)
const unsigned long HOME_TIMEOUT_MS = 0;   // vorher 15000UL
...
// *** NEU / ÄNDERUNG: HOME-Request auch während Fahrt
volatile bool force_home_request = false;

// *** NEU / ÄNDERUNG: Nach STOP zuerst Ziel fahren, dann Homing
...
// *** NEU / ÄNDERUNG: schneller Parser in blockierenden Loops (STOP_X + HOME_X) ***
    ...
    if (c == '\n' || c == '\r') {
      ...
      if (buf.length() > 0) {
        if (buf.equalsIgnoreCase("STOP_X")) {
          ...
          // HOME während Fahrt: erst stoppen, dann homing
...
bool doHomingSequenceBlocking() {
  stop_requested = false;
  force_home_request = false; // wenn wir hier sind, erfüllen wir die Home-Anforderung
  ...
  while (!endstopHit()) {
    pollSerialQuickForStopOrHome();                 // *** NEU / ÄNDERUNG
    if (stop_requested) {
      stopMotionImmediate();
      SerialUSB.println(F("HOMING: ABORTED BY STOP"));
      return false;
    }
    // Timeout nur prüfen, falls gesetzt
  ...
  // nach erfolgreichem Homing ist die Home-Lage frisch gesetzt
...
bool driveToPositionBlocking(long goal_steps) {
  ...
  while (true) {
    ...
    if (stop_requested) {
      stopMotionImmediate();
      // Wenn während Fahrt HOME angefordert wurde, jetzt direkt homing ausführen
...
void runPickSequence(float mm_target) {
  ...
  // Nach STOP: erst Ziel -> dann Homing
  if (require_home_after_next_action) {
    ...
    if (!doHomingSequenceBlocking()) {
      SerialUSB.println(F("PICK: WARN (kein HOMING nach Ziel)"));
      SerialUSB.println(F("REACHED"));
      return;
    }
...
void loop() {
      ...
      if (rxBuf.length() > 0) {
        ...
        if (line.equalsIgnoreCase("STOP_X")) {
          ...
          if (!isnan(mm)) {
            ...
            if (require_home_after_next_action) {
              if (!driveToPositionBlocking(tgt)) {
                SerialUSB.println(F("REACHED"));
              } else {
                doHomingSequenceBlocking();
                SerialUSB.println(F("REACHED"));
  ...
  if (stop_requested) {
    stopMotionImmediate();
    // Falls Home mitten in Idle angefordert wurde
    if (force_home_request) {
      doHomingSequenceBlocking();
      SerialUSB.println(F("REACHED"));
      force_home_request = false;
